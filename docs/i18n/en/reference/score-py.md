...
Scoring script used to serve predictions from the deployed Azure ML endpoint. Implements `init()` to load the registered model and `run()` to validate inputs and return predictions.

[⬇ Download score.py](../assets/files/score.py){ .md-button .md-button--primary download="score.py" }
